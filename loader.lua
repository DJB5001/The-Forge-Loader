-- The Forge Loader (Base64 Encoded Version)
-- Loads all files from The-Forge-Loader repository
...
local REPO_NAME = "The-Forge-Loader"
local BRANCH = "main"
...
local function loadModule(filename)
    ...
    local url = string.format(
        "https://raw.githubusercontent.com/%s/%s/%s/encoded/%s.b64?%d",
        REPO_OWNER,
        REPO_NAME,
        BRANCH,
        filename,
...
local url = string.format(
    "https://raw.githubusercontent.com/%s/%s/%s/encoded/loader.lua.b64?%d",
    REPO_OWNER,
    REPO_NAME,
    BRANCH,
    tick()
